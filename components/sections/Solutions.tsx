...
export default function SolutionsSection() {
  ...
  useEffect(() => {
    ...
    const engine = Engine.create({
      ...
      gravity: { x: 0, y: 1.5 } // Increased gravity for snappier fall
    });
    ...
    // Invisible walls to keep pills inside
    const wallThick = 200;
    ...
    PILLS_DATA.forEach((pill, index) => {
      ...
      const body = Bodies.rectangle(x, y, w, h, {
        ...
        friction: 0.1,
        frictionAir: 0.02, // Air resistance for smoother fall
        density: 0.005,
    ...
    // ✅ ALLOW SCROLL: Remove Matter.js mouse hijacking
  ...
  return (
    <div ref={containerRef} className="bg-[#0f172a] rounded-[2.5rem] py-8 px-6 md:px-10 my-8 text-white relative overflow-hidden border border-white/10 shadow-2xl">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-6 items-center">
        ...
        <div className="relative z-10">
          ...
          >
            ...
            {/* ✅ ONE LINE: Removed <br/> */}
            <h2 className="text-3xl md:text-4xl font-black font-serif leading-tight mb-2 text-white">
              Proven Excellence.
            </h2>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-4">
            {SOLUTIONS_STATS.map((stat, index) => (
              <motion.div
                ...
                className="bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10 hover:border-brand-main/50 hover:shadow-[0_0_15px_rgba(99,102,241,0.2)] transition-all cursor-default group"
              >
                ...
                <div className="text-[9px] font-bold text-white/50 uppercase tracking-wide">
                  {stat.label}
                </div>
              </motion.div>
            ))}
            ...
            <div className="flex flex-wrap gap-1.5">
              {TECH_STACK.map((tech, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -2 }}
                  className="px-2.5 py-1 bg-white/5 text-white rounded-full text-[10px] font-bold flex items-center gap-1.5 border border-white/10 hover:border-brand-main/30 shadow-sm transition-all"
                >
                  <span>{tech.icon}</span>
                  <span>{tech.name}</span>
                </motion.div>
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="bg-brand-main text-white px-5 py-2.5 rounded-full font-bold text-xs flex items-center gap-2 shadow-xl hover:shadow-2xl hover:shadow-brand-main/40 transition-all group"
          >
            Start Your Project
        ...
        >
          {/* ✅ SCROLL ENABLED: Removed touch-pan-y, added overflow-hidden to prevent scrollbars */}
          <div
            ref={canvasRef}
            className="absolute inset-0 bg-white/5 backdrop-blur-md rounded-[1.5rem] border border-white/10 shadow-inner overflow-hidden"
          >
            {/* Helper Text */}
            ...
            {PILLS_DATA.map((pill, index) => (
              <div
                ...
                className={cn(
                  "absolute top-0 left-0 rounded-full text-white shadow-lg whitespace-nowrap select-none z-50 border border-white/20 backdrop-blur-sm",
                  pill.color,
